...
export default {
  ...
  methods: {
    getPageList(){
      this.request.get('/api/blog/blogTag/admin/getBlogTagPage', {params:this.queryForm}).then(res =>{
        if(res.code === 200){
      ...
      this.$refs['dialogForm'].validate((valid) => {
        if (valid) {
          let url = this.dialogForm.categoryId == null ?
              '/api/blog/blogTag/admin/addBlogTag' : '/api/blog/blogTag/admin/updateBlogTag'
    ...
    deleteRows(id){
      ...
      this.$confirm('确定要删除此'+ids.length+'条数据吗?', '系统提示', {type : 'warning'}).then(() => {
        this.request.delete('/api/blog/blogTag/admin/deleteBatch', {data:ids}).then(res =>{
          if(res.code === 200){
